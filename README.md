...
Локальный пайплайн для распознавания исторических русскоязычных PDF и
...
PDF, изображения, результаты инференса, виртуальные окружения, notebook-файлы и
локальные batch-скрипты намеренно исключены из Git.
...
python3 -m py_compile \
  historical_russian_htr_pipeline.py \
  hybrid_htr_table_pipeline.py
```

Статический анализ и форматирование выполнялись Ruff:
